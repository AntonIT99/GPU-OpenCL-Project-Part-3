...
bool CConvolutionTaskBase::InitResources(cl_device_id , cl_context Context)
{
	...
	for(int i = 0; i < 3; i++)
	{
		m_hSourceChannels[i] = new float[Height * m_Pitch];
		m_hCPUResultChannels[i] = new float[Height * m_Pitch];
		m_hGPUResultChannels[i] = new float[Height * m_Pitch];
	}
	...
	for (unsigned int y = m_Height; y < Height; y++)
	{
		for (unsigned int x = 0; x < m_Pitch; x++)
		{
			m_hSourceChannels[0][pixelOffset] = 0.0f;
			m_hSourceChannels[1][pixelOffset] = 0.0f;
			m_hSourceChannels[2][pixelOffset] = 0.0f;
			pixelOffset++;
		}
	}

	unsigned int dataSize = m_Pitch * Height * sizeof(cl_float);
...
bool CConvolutionTaskBase::ValidateResults()
{
	...
	float avgError = 0;
	float maxError = 0;
	float numValues = float(numChannels * m_Width * m_Height);
